...
def ufl_element(triangle_names):
    return ufl.legacy.FiniteElement(triangle_names, ufl.triangle, 2)
...
def ufl_A(request, tensor_name):
    return ufl.legacy.FiniteElement(tensor_name, request.param, 1)


@pytest.fixture
def ufl_B(tensor_name):
    return ufl.legacy.FiniteElement(tensor_name, ufl.interval, 1)


def test_tensor_prod_simple(ufl_A, ufl_B):
    tensor_ufl = ufl.legacy.TensorProductElement(ufl_A, ufl_B)
...
def test_interval_variant_default(family, expected_cls):
    ufl_element = ufl.legacy.FiniteElement(family, ufl.interval, 3)
...
def test_interval_variant(family, variant, expected_cls):
    ufl_element = ufl.legacy.FiniteElement(family, ufl.interval, 3, variant=variant)
    assert isinstance(create_element(ufl_element), expected_cls)


def test_triangle_variant_spectral_fail():
    ufl_element = ufl.legacy.FiniteElement('DP', ufl.triangle, 2, variant='spectral')
    with pytest.raises(ValueError):
        create_element(ufl_element)


def test_triangle_variant_spectral_fail_l2():
    ufl_element = ufl.legacy.FiniteElement('DP L2', ufl.triangle, 2, variant='spectral')
    with pytest.raises(ValueError):
        create_element(ufl_element)


def test_quadrilateral_variant_spectral_q():
    element = create_element(ufl.legacy.FiniteElement('Q', ufl.quadrilateral, 3, variant='spectral'))
...
def test_quadrilateral_variant_spectral_dq():
    element = create_element(ufl.legacy.FiniteElement('DQ', ufl.quadrilateral, 1, variant='spectral'))
...
def test_quadrilateral_variant_spectral_dq_l2():
    element = create_element(ufl.legacy.FiniteElement('DQ L2', ufl.quadrilateral, 1, variant='spectral'))
    assert isinstance(element.element.A, FIAT.GaussLegendre)
    assert isinstance(element.element.B, FIAT.GaussLegendre)


def test_quadrilateral_variant_spectral_rtcf():
    element = create_element(ufl.legacy.FiniteElement('RTCF', ufl.quadrilateral, 2, variant='spectral'))
