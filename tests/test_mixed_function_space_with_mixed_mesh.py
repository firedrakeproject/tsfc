...
from tsfc.ufl_utils import compute_form_data, make_domain_integral_type_map
...
def test_mixed_function_space_with_mixed_mesh_restrictions_base():
    ...
    domain_integral_type_map, _ = make_domain_integral_type_map(integral_data, form_data)
    assert len(domain_integral_type_map) == 1
    assert domain_integral_type_map[mesh1] == "exterior_facet"
    # b
    form = inner(grad(f1('|')), grad(f1('|'))) * dS0(777)
    form_data = compute_form_data(form)
    integral_data, = form_data.integral_data
    domain_integral_type_map, _ = make_domain_integral_type_map(integral_data, form_data)
    assert len(domain_integral_type_map) == 2
    assert domain_integral_type_map[mesh0] == "interior_facet"
    assert domain_integral_type_map[mesh1] == "exterior_facet"
    # e
    ...
    domain_integral_type_map, _ = make_domain_integral_type_map(integral_data, form_data)
    assert len(domain_integral_type_map) == 2
    assert domain_integral_type_map[mesh0] == "cell"
    assert domain_integral_type_map[mesh1] == "cell"
...
def test_mixed_function_space_with_mixed_mesh_3_cg3_bdm3_dg2_dx1():
    ...
    integral_data, = form_data.integral_data
    domain_integral_type_map, _ = make_domain_integral_type_map(integral_data, form_data)
    assert len(domain_integral_type_map) == 3
    assert domain_integral_type_map[mesh0] == "cell"
    assert domain_integral_type_map[mesh1] == "cell"
    assert domain_integral_type_map[mesh2] == "cell"
