...
def test_ufl_only_simple():
    mesh = ufl.Mesh(ufl.legacy.VectorElement("P", ufl.triangle, 1))
    V = ufl.FunctionSpace(mesh, ufl.legacy.FiniteElement("P", ufl.triangle, 2))
    v = ufl.Coefficient(V)
    expr = ufl.inner(v, v)
    W = V
...
def test_ufl_only_from_contravariant_piola():
    mesh = ufl.Mesh(ufl.legacy.VectorElement("P", ufl.triangle, 1))
    V = ufl.FunctionSpace(mesh, ufl.legacy.FiniteElement("RT", ufl.triangle, 1))
    v = ufl.Coefficient(V)
    expr = ufl.inner(v, v)
    W = ufl.FunctionSpace(mesh, ufl.legacy.FiniteElement("P", ufl.triangle, 2))
    to_element = create_element(W.ufl_element())
    kernel = compile_expression_dual_evaluation(expr, to_element, W.ufl_element())
    assert kernel.needs_external_coords is True
...
def test_ufl_only_to_contravariant_piola():
    ...
    W = ufl.FunctionSpace(mesh, ufl.legacy.FiniteElement("RT", ufl.triangle, 1))
    to_element = create_element(W.ufl_element())
    kernel = compile_expression_dual_evaluation(expr, to_element, W.ufl_element())
    assert kernel.needs_external_coords is True
