...
class KernelBuilder(KernelBuilderBase, KernelBuilderMixin):
    ...
    def set_domain_integral_type_map(self, domain_integral_type_map, all_meshes):
        self._domain_integral_type_map = {mesh: domain_integral_type_map[mesh] if mesh in domain_integral_type_map else None for mesh in all_meshes}
    ...
    def set_arguments(self):
        ...
        return_variables = prepare_arguments(arguments,
                                             ...
                                             self.integral_data_info.argument_integral_types,
    ...
    def set_coordinates(self, domains):
        ...
        :arg domains: Sequence of :class:`ufl.Domain`s
        """
            ...
            if integral_data.enabled_coefficients[i]:
                ...
                if type(coefficient.ufl_element()) == ufl_MixedElement:
                    if original in self.dont_split:
                        ...
                        for j, (mesh, element) in enumerate(zip(mixed_mesh, coefficient.ufl_element().sub_elements, strict=True)):
    ...
    def make_coefficient_gem_expressions(self):
        # This needs to be called after domain_integral_type_map is set.
        for k, coeff in enumerate(self.coefficient_number_index_map):
    ...
    def make_active_domain_numbers(self, domain_expr_dict, active_variables, kernel_arg_type, dtype=None):
        # domain_coeff_dict is ordered as collect_domains_in_form(sub_form).
        active_dns = []
        args = []
